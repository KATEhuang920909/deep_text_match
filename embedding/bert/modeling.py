...
class BertModel(object):
    ...
    def __init__(self,
        ...
        if input_mask1 is None:
            input_mask1 = tf.ones(shape=[batch_size, seq_length], dtype=tf.int32)
        if input_mask2 is None:
            input_mask2 = tf.ones(shape=[batch_size, seq_length], dtype=tf.int32)

        if token_type_ids1 is None:
            token_type_ids1 = tf.zeros(shape=[batch_size, seq_length], dtype=tf.int32)
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("embeddings"):
                ...
                (self.embedding_output1, self.embedding_table1) = embedding_lookup(
                ...
                self.embedding_output1 = embedding_postprocessor(
            ...
            #  embedding of sentence2
            with tf.variable_scope("embeddings", reuse=True):
                # Perform embedding lookup on the word ids.
                (self.embedding_output2, self.embedding_table2) = embedding_lookup(
                ...
                # Add positional embeddings and token type embeddings, then layer
                # normalize and perform dropout.
                self.embedding_output2 = embedding_postprocessor(
